 ...
 *
 * Heavy bits (codebase walk, regex scans) all live behind [ReadAction.compute]
...
class DesignSystemAnalyzer(private val project: Project) {
    ...
    fun analyze(scopeFile: VirtualFile? = null): AnalysisReport {
        ...
        val tokens = ReadAction.compute<List<DesignToken>, RuntimeException> {
            TokenIndex.getInstance(project).get(scopeFile)
        }
    ...
    private fun computeCoverage(tokens: List<DesignToken>): CoverageScan {
        ...
        for (vf in files) {
            val text = try {
                ReadAction.compute<CharSequence, RuntimeException> { VfsUtilCore.loadText(vf) }
